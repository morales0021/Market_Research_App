...
// import contentKGDSection from '@/tmp_data/subsection_kgd.json';
import { useLocation, useNavigate } from 'react-router-dom';
import ResearchReport from '@/sections/template_section_2';
import { MenuIcon, XIcon } from '@heroicons/react/outline'; // Import icons
...
export default function DisplayPage() {
  ...
  // Redirect to home if no industry or region is provided
  if (!industry || !region) {
    navigate('/');
    return null;
  }

  const contentKGDSection = async () => {
    const response = axios.post("http://192.168.1.48:8000/subsection",
    {
      industry: industry,
      region: region,
    ...
    <div className="flex min-h-screen relative">
      ...
      >
        {menuOpen ? (
          ...
          <MenuIcon className="h-6 w-6" /> // Open menu icon
        )}
      </button>
      ...
      <div className="flex-1 p-4 ml-0 md:ml-10 lg:ml-80 max-w-[1100px] mt-4">
        ...
        {activeSection === 'Key Growth and Challenges' && <ResearchReport contentSection={contentKGDSection} />}
      </div>
    </div>
  );
}
